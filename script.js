...
const movieData = movies.Search.map((item) => {
    return `
    <div class="card p-0 shadow" style="width: 18rem">
        <img src="${item.Poster}" class="card-img-top" style="height: 400px; object-fit: cover;" alt="${item.Title}" />
        <div class="card-body">
            <h5 class="card-title text-center">${item.Title}</h5>
        </div>
        <ul class="list-group list-group-flush">
            <li class="list-group-item"><b>Year:</b> ${item.Year}</li>
            <li class="list-group-item"><b>Rating:</b> ${item.imdbID}</li>
            <li class="list-group-item"><b>Type:</b> ${item.Type}</li>
        </ul>
    </div>
    `
}).join("")
